...
int main() {
    ...
    function cvtcolor_dist("cvtcolorautodist0_tiramisu");

    global::set_number_of_ranks(NODES);

    cvtcolor_dist.add_context_constraints("[COLS]->{:COLS = " + std::to_string(_COLS) + "}");
    ...
    input.split(j, _COLS / NODES, i1, i2);
    input.tag_distribute_level(i1);
    input.drop_rank_iter(i1);

    RGB2Gray_s0.split(j, _COLS / NODES, i1, i2);
    RGB2Gray_s0.tag_distribute_level(i1);
    RGB2Gray_s0.drop_rank_iter(i1);
    RGB2Gray_s0.tag_parallel_level(i2);

    buffer buff_input("buff_input", {_ROWS , COLS/NODES, 3}, p_uint8, a_input,
                      &cvtcolor_dist);
    buffer buff_RGB2Gray("buff_RGB2Gray", {_ROWS , COLS/NODES}, p_uint8, a_output,
                         &cvtcolor_dist);
    ...
    cvtcolor_dist.codegen({&buff_input, &buff_RGB2Gray}, "build/generated_fct_cvtcolorautodist0.o");
