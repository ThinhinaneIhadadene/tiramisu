...
{
    tiramisu::init("convolutionautodist0_tiramisu");

    function *f = global::get_implicit_function();

    global::set_number_of_ranks(_NODES);

    f->add_context_constraints("[COLS]->{: COLS = "+std::to_string( _COLS)+"}");
    ...
    var i01("i01"), i02("i02");
    in.split(i1, _ROWS/_NODES, i01, i02);
    in.tag_distribute_level(i01);
    in.drop_rank_iter(i01);

    conv.split(y, _ROWS/_NODES, i01, i02);
    conv.tag_distribute_level(i01);
    conv.drop_rank_iter(i01);

    buffer buff_input("buff_input", {_ROWS, _COLS / _NODES + 2, _CHANNELS}, p_int32, a_input);
    buffer buff_kernel("buff_kernel", {kernel_extent_1, kernel_extent_0}, p_float32, a_input);
    buffer buff_convolution("buff_convolution", {_ROWS, _COLS/ _NODES, _CHANNELS}, p_int32, a_output);
    ...
    tiramisu::codegen({&buff_input, &buff_kernel, &buff_convolution} ,"build/generated_fct_convolutionautodist0.o");
